...
1. Back up the existing vault (`tar cz` or a git commit) before anything else.
2. Add the two core notes (`<your-name>.md`, `memexlab.md`) if missing.
3. Run `memex doctor` against the vault. Fix any folder-shape issues it reports.
4. Run `memex migrate` (dry-run first, then `--apply`) to bring existing frontmatter up to schema.
5. Run `memex lint`. Expect errors in existing notes whose frontmatter predates the schema; fix them one file at a time.
...
1. **You can answer questions about your domain faster than before.** "What do I think about X?" completes in the vault in under a minute.
2. **Your outputs sound more like you, more quickly.** The first draft from `memex export essay` is 70% there; you used to start from 0%.
3. **You notice gaps before meetings.** Prep reveals "I don't have a solid position on Y" while there's still time to develop one.
